...
// ─── In-memory store ──────────────────────────────────────────────────────────
// cameras: { [cam_id]: { id, name, running, fps, total_people, counts, error, updatedAt } }
const cameras = {};
...
// ─── Auth helper ──────────────────────────────────────────────────────────────
function checkApiKey(req, res, next) {
  if (!API_KEY) return next();                      // ไม่ได้ตั้งค่า = เปิดไว้
  const key = req.headers['x-api-key'];
  if (key !== API_KEY) {
...
// ─── Push endpoint (on-site server → gateway) ────────────────────────────────
/**
 * POST /api/push
 * Body: array of camera objects (เหมือน response ของ GET /api/cameras บน FastAPI)
 * [
 *   { id, name, running, fps, total_people, counts, error },
 *   ...
 * ]
 */
app.post('/api/push', checkApiKey, (req, res) => {
  ...
  if (!Array.isArray(body)) {
    return res.status(400).json({ error: 'Body must be an array of camera objects' });
  }
  const now = Date.now();
  body.forEach(cam => {
    if (!cam.id) return;
...
// ─── Read endpoints (front end → gateway) ────────────────────────────────────
/**
 * GET /api/cameras
 * Returns all cameras (marks stale ones as offline)
 */
app.get('/api/cameras', (req, res) => {
  const now = Date.now();
  const result = Object.values(cameras).map(cam => {
    const stale = (now - cam.updatedAt) > STALE;
    return {
      ...cam,
      running:   stale ? false : cam.running,
      error:     stale ? `ไม่ได้รับข้อมูลนาน ${Math.round((now - cam.updatedAt) / 1000)} วินาที` : cam.error,
    };
  });
  res.json(result);
});
...
/**
 * GET /api/cameras/:cam_id/status
 * Same fields as FastAPI status endpoint
 */
app.get('/api/cameras/:cam_id/status', (req, res) => {
  const cam = cameras[req.params.cam_id];
  if (!cam) {
    return res.json({ running: false, fps: 0, total_people: 0, counts: {}, error: 'ไม่พบกล้อง', has_frame: false });
  }
  const stale = (Date.now() - cam.updatedAt) > STALE;
  res.json({
    running:      stale ? false : cam.running,
...
/**
 * GET /api/summary
 * Grand total across all cameras (ใช้แสดงบน dashboard ง่ายๆ)
 */
app.get('/api/summary', (req, res) => {
  const now = Date.now();
  let grandTotal = 0;
  ...
  Object.values(cameras).forEach(cam => {
    const stale = (now - cam.updatedAt) > STALE;
    if (!stale && cam.running) {
      grandTotal += cam.total_people || 0;
      liveCams++;
    }
  });
  res.json({
    ...
    live_cams:   liveCams,
    total_cams:  Object.keys(cameras).length,
...
app.listen(PORT, () => {
  console.log(`[Gateway] listening on port ${PORT}`);
  console.log(`[Gateway] API_KEY auth: ${API_KEY ? 'enabled' : 'disabled (set API_KEY env var to enable)'}`);
  console.log(`[Gateway] stale timeout: ${STALE / 1000}s`);
});
